...
void calculateIndividualParticleForces(size_t firstIndex, size_t secondIndex, std::vector<std::vector<double> > & forces, std::vector<position3D<double> > & r, simBox<double> & sb, bool pbc, std::vector<bool> & isCrossover, double beadAxialSeparation, double beadRadialSeparation, double epsilon, double sigma, const size_t n_part, const size_t n_scaf, const size_t n_stap, std::vector<int> & belongsTo, std::vector<size_t> & isBound, std::vector<std::vector<int> > & staple_connections, double RCUT,  bool CIRCULAR_SCAFFOLD, std::ofstream & btout, size_t t, size_t & n_bound_staples, bool PRE_RUNGE_KUTTA, std::vector<int> & StrandNumber, std::ofstream & fbtOut, std::vector<size_t> & prevBound, double bind_energy_kcal_mol, double BIND_CUTOFF, bool FORCED_BINDING, std::vector<std::mutex> & mtx){
  ...
  for (size_t i = firstIndex; i <= secondIndex; ++i){
    ...
    //////////////////////////////////////////////////////////
    // SCAF-STAP EXCLUDED VOLUME ////////////////////////////
    ////////////////////////////////////////////////////////
    
    if (i < n_scaf){
      for (size_t j = n_scaf; j < n_part; ++j){
    	if (j!=belongsTo[i]){
    	  distances(pbc,sb,RXI,RYI,RZI,RXIJ,RYIJ,RZIJ,DIST,r,j);
	  if (DIST > RCUT) {
	  } else {
    	    FORCE = excvolwca(sigma, exc_vol_const/10,DIST);
    	    addPairForces(i,j,forces,FORCE,RXIJ,RYIJ,RZIJ,DIST, mtx);
	  }
    	}
      }
    }
    ...
    // SCAFFOLD EXCLUDED VOLUME
    for (size_t j = i+2; j < n_scaf; j++){
      distances(pbc,sb,RXI,RYI,RZI,RXIJ,RYIJ,RZIJ,DIST,r,j);
      if (RXIJ>RCUT || RYIJ>RCUT || RZIJ>RCUT){ //MOST LIKELY AND FASTEST
	// NO FORCE
      } else {
	if (isBound[i] && isBound[j] && verbose){
	  FORCE = excvolwca(sigma, exc_vol_const,DIST);
	  if (t%1000==0 && FORCE!=0 && excVerbose){
	    std::cout << i << ", " << j << "; excvol " << FORCE << " pN at " << DIST << "nm.\n"; 
	  }
          addPairForces(i,j,forces,FORCE,RXIJ,RYIJ,RZIJ,DIST, mtx);
	} else {
	  FORCE = excvolwca(sigma, exc_vol_const,DIST);
    ...
    } //end "j" loop for nonbonded repulsion
    ...
    if (i < n_scaf){
      ...
      //STAPLE-STAPLE INTERACTIONS: NO EXC VOL, JUST CONSECUTIVE BACKBONE CONSTRAINTS
      if (i < n_part - 1){
        if (staple_connections[i][i+1]){
	  distances(pbc,sb,RXI,RYI,RZI,RXIJ,RYIJ,RZIJ,DIST,r,i+1);
	  if (isCrossover[i] && isCrossover[i+1] && StrandNumber[i] !=StrandNumber[i+1] && isBound[i] && isBound[i+1]){
	    FORCE = simpleHarmonic(beadRadialSeparation,DIST,k_stretch);
	    addPairForces(i,i+1,forces,FORCE,RXIJ,RYIJ,RZIJ,DIST, mtx);
          } else {
        ...
        } //end of if (staple_connections) loop
	//EXCLUDED VOLUME OF STAPLES IF BOUND
	for (size_t j = i+2; j < n_part; j++){
...
void calculatePairForces(std::vector<std::vector<double> > & forces, std::vector<position3D<double> > & r, simBox<double> & sb, bool pbc, std::vector<bool> & isCrossover, double beadAxialSeparation, double beadRadialSeparation, double epsilon, double sigma, const size_t n_part, const size_t n_scaf, const size_t n_stap, std::vector<int> & belongsTo, std::vector<size_t> & isBound, std::vector<std::vector<int> > & staple_connections, double RCUT,  bool CIRCULAR_SCAFFOLD, std::ofstream & btout, size_t t, size_t & n_bound_staples, bool PRE_RUNGE_KUTTA, std::vector<int> & StrandNumber, std::ofstream & fbtOut, std::vector<size_t> & prevBound, double bind_energy_kcal_mol, double BIND_CUTOFF, bool FORCED_BINDING, std::vector<std::mutex> & mtx){
  ...
  for (size_t i = 0; i < NTHREADS; ++i){
    ...
    threads.push_back(std::thread(calculateIndividualParticleForces,arg1,arg2, std::ref(forces), std::ref(r), std::ref(sb), pbc, std::ref(isCrossover), beadAxialSeparation, beadRadialSeparation, epsilon, sigma, n_part, n_scaf, n_stap, std::ref(belongsTo), std::ref(isBound), std::ref(staple_connections), RCUT, CIRCULAR_SCAFFOLD, std::ref(btout), t, std::ref(n_bound_staples), PRE_RUNGE_KUTTA, std::ref(StrandNumber), std::ref(fbtOut), std::ref(prevBound), bind_energy_kcal_mol, BIND_CUTOFF, FORCED_BINDING,std::ref( mtx)) );
