...
void recordFrame(std::string & OUT_TYPE_, std::ofstream & traj, std::vector<position3D<double> > & R, size_t n_part, size_t n_scaf, std::vector<int> & stapleNumbers_, size_t t_, simBox<double> & simbox, std::vector<size_t> & isBound){
  ...
  if (OUT_TYPE_=="LAMMPS"){
    ...
    for (size_t i = n_scaf; i < n_part; i++){
       ...
       //xpos[i] += centroidX-0.5;
       //ypos[i] += centroidY-0.5;
       //zpos[i] += centroidZ-0.5;
    }
	...
	//std::cout << " Current coords for " << i << " are " << R[i].x << ", " << R[i].y << ", " << R[i].z << ".\n";
	//std::cout << " Written coords are " << xpos[i] << " " << ypos[i] << " " << zpos[i] << ".\n";
	//std::cout << " Translates to " << (xpos[i]-0.50)*simbox.dimensions.x << " " << (ypos[i]-0.50)*simbox.dimensions.y << " " << (zpos[i]-0.5)*simbox.dimensions.z << ".\n";
